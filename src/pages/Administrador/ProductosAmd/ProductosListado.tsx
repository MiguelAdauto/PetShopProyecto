...
const datosProductos = Array.from({ length: 23 }, (_, index) => ({
  ...
  precioCompra: parseFloat((Math.random() * 100).toFixed(2)),
  precioVenta: parseFloat((Math.random() * 100).toFixed(2)),
  precioTotal: parseFloat((Math.random() * 100).toFixed(2)),
  tipo: ["Mixto", "Perro", "Gato"][index % 3],
  categoria: ["Juguetes", "Aseo", "Accesorios"][index % 3],
...
const ProductosListado = () => {
  ...
  const navigate = useNavigate(); // ✅ para navegar al formulario
